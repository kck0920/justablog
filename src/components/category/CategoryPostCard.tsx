  ...
  return (
    <Link to={`/post/${post.slug}`} className="group">
      <Card className={
         ...
         `${isAllPostsView ? "flex flex-col md:flex-row" : ""} ` +
         `hover:bg-muted/50 dark:hover:bg-slate-850`
      }>
        {/* {`h-full overflow-hidden transition-all hover:shadow-md ${isAllPostsView ? "flex flex-col md:flex-row" : ""}`}> */}
        {isAllPostsView && (
